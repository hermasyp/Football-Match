...
import com.catnipdev.footballmatch.data.local.FavoriteEvent
...
data class Event(
	...
	companion object {
	    fun parseEvent(listFavorite : List<FavoriteEvent>) : List<Event>?{
			var listEvent : MutableList<Event>? = arrayListOf()
			listFavorite.forEach { fe ->
				listEvent?.add(Event(idEvent = fe.idEvent,
						idLeague = fe.idLeague,
						dateEvent = fe.dateEvent,
						intHomeScore = fe.intHomeScore,
						intAwayScore = fe.intAwayScore,
						strHomeTeam = fe.strHomeTeam,
						strAwayTeam = fe.strAwayTeam))
			}
			Log.d("EVENT", "total data after parse:" + listEvent?.size)
